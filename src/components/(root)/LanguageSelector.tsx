...
import { LANGUAGE_CONFIG } from "@/constants/(root)";
...
//Unlocked Languages
const unlockedLanguages = ["python", "java"];
...
function LanguageSelector({ hasAccess }: { hasAccess: boolean }) {
  ...
  const handleLanguageSelect = (langId: string) => {
    if (!hasAccess && !unlockedLanguages.includes(langId)) return;
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      rounded-lg transition-all 
       ...
       ${!hasAccess && !unlockedLanguages.includes(language) ? "opacity-50 cursor-not-allowed" : ""}`}
          ...
          >
            ...
            <div className="max-h-[280px] overflow-y-auto overflow-x-hidden">
              {Object.values(LANGUAGE_CONFIG).map((lang, index) => {
                const isLocked = !hasAccess && !unlockedLanguages.includes(lang.id);
